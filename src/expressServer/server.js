...
router.get("/tripupdates/:agencyId(\\d+)", (req, res, next) => {
  ...
  const translocCall = webCallUtils.createTranslocCall(
    agencyId,
    constants.defaultTranslocAPIKey
  );

  translocCall.end(function (translocRes) {
    const feedMessage = webCallUtils.getTripUpdateFeedMessage(
      translocRes,
      agencyId
    );
...
router.get("/vehiclepositions/:agencyId(\\d+)", (req, res, next) => {
  ...
  translocCall.end(function (translocRes) {
    const feedMessage = webCallUtils.getVehiclePositionFeedMessage(
      translocRes,
      agencyId
    );
    ...
    res.end(encodedMessage);
    console.log(
      "Sent Vehicle Positions of size " +
        encodedMessage.length +
        " for agency " +
        agencyId +
        " at " +
